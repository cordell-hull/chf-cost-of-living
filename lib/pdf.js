...
export class PDFGenerator {
  ...
  async generate(report, onProgress = () => {}) {
    ...
    if (report.healthInsurance.schoolPaysPremium) {
      this.drawCostRow('Monthly premium', 'ZERO, because the school pays for your health insurance');
    } else if (report.healthInsurance.monthlyPremiumEstimate) {
    ...
    this.drawWrappedText(
      'The school is required to pay the Cordell Hull Foundation sponsorship fees of $1,400 for the first year, $800 for the second and third years. The teacher must not pay these fees.',
      this.margins.left + 5, this.contentWidth - 10,
    ...
    if (report.visaFees.integrityFee) {
      const intNote = report.visaFees.integrityFee === 'teacher'
        ? 'Charged to teacher credit card; reimbursed after leaving the US'
        : 'Reimbursed by school';
      this.drawCostRow('Visa Integrity Fee ($250)', intNote);
    }
    ...
    this.addSpace(5);
    if (report.housing.relocationHired === true) {
      this.drawCostRow('Relocation company', report.housing.relocationCompany || 'Yes');
      if (report.housing.relocationEmail) this.drawCostRow('Company contact', report.housing.relocationEmail);
    } else if (report.housing.relocationHired === false) {
      this.drawCostRow('Relocation company', 'No');
    }

    const svcLabels = { streeteasy: 'StreetEasy', zillow: 'Zillow', renthop: 'Rent Hop', other: 'Other' };
    const svcs = (report.housing.apartmentServices || [])
      .map(s => s === 'other' && report.housing.apartmentOther ? report.housing.apartmentOther : svcLabels[s] || s)
      .filter(Boolean);
    if (svcs.length) this.drawCostRow('Apartment search', svcs.join(', '));
    ...
    this.addSpace(15);
    this.drawWrappedText(
      'For further information, all costs associated with the J-1 visa are listed on the Cordell Hull Foundation website: www.cordell-hull.org/j1-visa-teacher-cost',
      this.margins.left + 5, this.contentWidth - 10,
      { fontSize: this.fonts.body, color: this.colors.accent }
    );
